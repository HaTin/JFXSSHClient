...
本项目已定稿为 **v1.0.0** 发布版本，作为最终形态交付。

- **V1（核心）+ V1.1（增强）**：全部完成。
- **V2 SFTP**：核心完成（双栏浏览、并发传输、文件夹上传、覆盖确认）。
- **V3 端口转发**：完成（本地 / 远程 / 动态、持久化、后台运行、自动启动、全部启动/停止）。
- **V4 Docker / V5 AI 助手**：不在本次交付范围（见文末「未来可扩展」）。

本文档为各功能的最终交付清单；带「未来可扩展」标注的条目为范围外的可选增强，非本版本待办。
...
- 单元测试：JUnit 5 + surefire，73 个用例（cipher / vault / settings / i18n /
  connection / group + 级联 / ssh 集成（密码 + 公钥，含内存私钥内容认证）/ tty-connector
  断开重连 / SFTP 浏览·传输·取消·多通道·错误码·statEntry / 端口转发本地+动态 + 持久化 +
  后台服务），`mvn test` 全绿（其中权限错误用例在 root 环境自动跳过）。
...
- **一次性命令执行 execute()**：API.md 预留的非交互入口，最终形态未实现（仅交互式 openShell）。

---

## 产品需求 V2–V5 交付情况

- **V2 SFTP**：核心完成（双栏文件管理器，含并发传输与进度/取消）。
  ...
  - 未来可扩展（范围外）：**文件夹整体下载**（右栏选目录提示"请选择文件"）、**下载侧同名覆盖确认**、
    **取消单个传输后目标栏不自动刷新**、断点续传、传输队列（并发数上限/暂停/重排）、拖拽。
- **V3 Port Forward**：已完成。
  - 支持本地转发（-L）、远程转发（-R）、动态 SOCKS 代理（-D）。
  - 独立窗口管理规则：表格展示名称/类型/绑定/目标/状态；工具栏添加/启动/停止/编辑/移除/全部启动/全部停止。
  ...
  - 未来可扩展（范围外）：规则批量导入/导出、远程转发集成测试。
- **V4 Docker**（未来可扩展，不在本次交付范围）：容器列表 / 详情 / 日志
- **V5 AI 助手**（未来可扩展，不在本次交付范围）：右侧可折叠聊天面板
- 插件系统（Plugin Manager）：不在产品需求范围内，不实现；Tools 菜单原占位项已移除。
